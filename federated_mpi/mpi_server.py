...
def run_server(comm):
    ...
    for round_num in range(1, 6):
        print(f"\n===== Round {round_num} =====")
        print("Broadcasting global model to all clients...")
        ...
        for i in range(1, num_clients + 1):
            print(f"Waiting for model weights from client {i}...")
            received = comm.recv(source=i, tag=i)
            print(f"Received weights from client {i}")
        ...
        print("Evaluating updated global model on test set...")
        loss, accuracy = model.evaluate(x_test, y_test, verbose=0)
        print(f"Round {round_num} Evaluation — Accuracy: {accuracy:.4f}, Loss: {loss:.4f}")
